...
COPY . .
RUN make submodules
RUN cd ./packages/contracts-bedrock && just -v build
...
ENV DEPLOYMENT_OUTFILE=/packages/contracts-bedrock/deployments/addresses.json
ENV DEPLOY_CONFIG_PATH=/packages/contracts-bedrock/deploy-config/deploy-config.json
ENV STATE_DUMP_PATH=/packages/contracts-bedrock/'allocs-l2.json' 
...
CMD echo "Updating deploy config..." && \
  ...
  cd /packages/contracts-bedrock && forge script scripts/deploy/Deploy.s.sol:Deploy --sig 'runWithStateDump()' --sender "${DEPLOYER_ADDRESS}" --private-key "${DEPLOYER_PRIVATE_KEY}" --gas-estimate-multiplier ${GAS_MULTIPLIER} --rpc-url "${ETH_RPC_URL}" --broadcast ${SLOW_ARG} && \
  cp ${DEPLOYMENT_OUTFILE} ${OUT_ADDRESSES} && \
  cp ${DEPLOY_CONFIG_PATH} ${OUT_DEPLOY_CONFIG} && \
  echo 'Generating L2 genesis allocs' && \
  cd /packages/contracts-bedrock && forge script scripts/L2Genesis.s.sol:L2Genesis --sig 'runWithAllUpgrades()' && mv ${STATE_DUMP_PATH} ${OUT_ALLOCS} && \
  echo 'Generating l2 genesis and rollup configs' && \
  cd ${OP_NODE_PATH} && go run cmd/main.go genesis l2 \
      --l1-rpc ${ETH_RPC_URL} \
